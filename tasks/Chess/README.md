...
其余私有方法负责进入对局、等待回目、处理 Buff、主动退出和返回大厅。
...
- `备`
  1. 如果出现 Buff 面板，立即中断其他操作并完成 Buff 选择。
  2. 打开商店扫描一次并购买阵容卡。
...
回目数需要连续两帧发生相同变化才确认进入下一回目。`ROUND` 和 `CHESS_MODE` 连续三帧为空后会重新截图；只有阵容入口也不存在时才确认对局结束。
