...
try:
    ...
    def _frombuffer(payload, dtype=None):
        return _FakeImage(payload, (len(payload), 1, 1))

    def _cvt_color(image, code):
        height, width, channels = image.shape
        pixels = [
            image.payload[offset:offset + channels]
            for offset in range(0, len(image.payload), channels)
        ]
        if code == 1:
            converted = b"".join(pixel[::-1] for pixel in pixels)
        elif code == 2:
            converted = b"".join(pixel[:3] for pixel in pixels)
        else:
            raise AssertionError(f"unexpected color conversion code: {code!r}")
        return _FakeImage(converted, (height, width, 3))
